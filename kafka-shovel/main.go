...
func main() {
	...
	r.POST("/test", func(c *gin.Context) {
		kafkaConfig := kafka.ConnectionParameters{
			ConsumerGroupID: config.GroupName,
			Conf:            KafkaConfig(config.KafkaVersion, "kafkaShovelClient"),
			Brokers:         config.Brokers,
			Topics:          []string{shovels[0].From},
		}
